...
/* Gyro Pitch/Roll/Yaw, Battery Voltage, Acc X/Y/Z values */
// vs16 ADCValues[7] = { 0, };
...
static void ADXL_Init(void)
{
    ...
    ADXL_WriteByte(ADXL_RATE_800 & 0x0F);
    ADXL_OFF;
    ADXL_ON;
...
static void ADXL_GetAccelValues(void)
{
    u8 i;
...
@near @interrupt void ADC1_IRQHandler(void)
{
    ...
    for (i = 0; i < 4; i++) {
        ...
        // TODO remove
        // ADCValues[i] = ADC1_GetBufferValue(i);
    }
...
void Sensors_Read(void)
{
    ...
    // accel
    acc[0] = sensorInputs[4] >> 3;
    acc[1] = sensorInputs[5] >> 3;
    acc[2] = sensorInputs[6] >> 3;
